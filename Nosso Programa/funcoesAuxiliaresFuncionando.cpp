...
void estampas(char tipo)
{
  ...
  //  cout<<"corrente no capacitor = "<<netlist[i].jt0<<endl;
  //  cout<<"tensao no capacitor = "<<netlist[i].vt0<<endl<<endl;
  //  getch();
  }
  else if (tipo=='L') {
    if (  analisandoPontodeOp ==1){
...
int  ComparaValorNR (void) {
  ...
  for (i=1; i<=nv; i++){
    if ((fabs(Yn[i][nv+1] - NewtonRaphsonVetor[i])) > MAX_ERRO_NR ) {
      erroGrande=1;
    //  cout<<"Erro atual no MontarEstampas"<< fabs(Yn[i][nv+1] - NewtonRaphsonVetor[i])<<endl;
...
int analiseNR ()
{
  ...
  ultimogs = CONDUTANCIA_INICIAL_GS;
  fazendoGminStepping=0;
//cout<<"AQUI TENTAR NOVAMENE = "<< NewtonRaphsonTentarNovamente<<endl;
    for (NewtonRaphsonTentarNovamente = 1; NewtonRaphsonTentarNovamente<=NEWTONRAPHSON_NUMERO_MAX_TENTARNOVAMENTE; NewtonRaphsonTentarNovamente++)
    {
      zeraSistema();
      montarEstampas();
